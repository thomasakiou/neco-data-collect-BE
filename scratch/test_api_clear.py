...
def test_running_api_clear_field():
    ...
    # Update SSCE with None (simulating frontend sending null)
    payload = json.dumps({"sch_email": None}).encode("utf-8")
    req = urllib.request.Request(f"{base_url}/ssce/{ssce_id}", data=payload, headers=headers, method="PUT")
    
    with urllib.request.urlopen(req) as response:
        print(f"Update response (null): {response.status}")
    ...
    with urllib.request.urlopen(req) as response:
        ...
        print(f"Fetch after null update sch_email: {sch_email}")

if __name__ == "__main__":
    test_running_api_clear_field()
